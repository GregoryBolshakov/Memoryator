...
public:
    ...
    void draw(RenderTarget& target, std::vector<DrawableChainElement*> drawableItems, float scale = 1, Vector2f cameraPosition = {0, 0});
	static std::vector<DrawableChainElement*> UpcastChain(std::vector<SpriteChainElement*> chain);
	static std::vector<SpriteChainElement*> DowncastToSpriteChain(std::vector<DrawableChainElement*> chain);
