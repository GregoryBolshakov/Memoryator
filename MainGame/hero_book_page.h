...
public:
	...
	std::unordered_map<Tag, book_object_info>* getObjectInfo() { return &objectInfo; }
	...
	page_content getPreparedContent(int pageNumber, Tag currentDraft = Tag::emptyDraft);
...
private:
	...
	std::unordered_map<AuxiliarySpriteTag, board_sprite> auxiliarySpriteMap;
	text_system textWriter;

	// object information (creatures, craft, plants, wreaths, nightmare)
	Vector2f getConnectionPosition(int numberInOrder);	

	std::unordered_map<Tag, book_object_info> objectInfo;
