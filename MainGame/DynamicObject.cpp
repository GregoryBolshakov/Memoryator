...
Vector2f DynamicObject::ellipseSlip(DynamicObject *dynamic, Vector2f newPos, Vector2f destination, Vector2f f1, Vector2f f2, float ellipseSize, float height, long long elapsedTime) const
{
	...
	{
		float k = ((dynamic->getMoveSystem().speed * elapsedTime) / sqrt(pow(x1 - dynamic->getPosition().x, 2) + pow((height - yl) - dynamic->getPosition().y, 2)));
	...
	{
		float k = ((dynamic->getMoveSystem().speed * elapsedTime) / sqrt(pow(x2 - dynamic->getPosition().x, 2) + pow((height - y2) - dynamic->getPosition().y, 2)));
...
void DynamicObject::setMoveOffset(long long elapsedTime)
{
	...
	if (movePosition == Vector2f(-1, -1))
	{
		...
		{
			const auto angle = int(directionSystem.direction) * pi / 180;
			const float k = moveSystem.speed * elapsedTime / sqrt(pow(cos(angle), 2) + pow(sin(angle), 2));
	...
	const auto k = moveSystem.speed * elapsedTime / sqrt(pow(movePosition.x - position.x, 2) + pow(movePosition.y - position.y, 2));
		...
		if (terrain->isMultiEllipse)
		{
			...
			if (terrain->isIntersected(this->getPosition(), newPosition))
			{
				...
				if (staticItem->isDotsAdjusted)
					motionAfterSlipping = terrain->newSlippingPositionForDotsAdjusted(this->getPosition(), moveSystem.moveOffset, moveSystem.speed, elapsedTime);
				else
