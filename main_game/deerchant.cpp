...
void deerchant::jerk(const float power, const float deceleration, Vector2f)
{
	...
	move_system.lax_move_position = Vector2f(position_.x + cos(float(direction_system.direction) * pi / 180) * jerk_distance_, position_.y - sin(float(direction_system.direction) * pi / 180) * jerk_distance_);
}
