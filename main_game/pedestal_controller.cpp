...
{
	filed_figure_.setSize(Vector2f(size_));
...
void pedestal_controller::write_to_file()
{
	...
	const Vector2f offset = { (bound_object_->get_texture_offset().x + center_position_.x - bound_object_->get_position().x) / bound_object_->get_texture_size().x,
...
void pedestal_controller::handle_events(Event & event)
{
	if (!bound_object_)
		return;
...
void pedestal_controller::interact(long long elapsed_time, Event event)
{
	...
	const auto mouse_world_pos = Vector2f((mouse_pos.x - helper::GetScreenSize().x / 2 + camera_position_.x * scale_factor_) / scale_factor_,
		(mouse_pos.y - helper::GetScreenSize().y / 2 + camera_position_.y * scale_factor_) / scale_factor_);

	// selecting focus to move
	if (!selected_center_ && selected_focus_ == -1 && (Mouse::isButtonPressed(Mouse::Left) || Mouse::isButtonPressed(Mouse::Right)))
	{
	...
	// selecting center
	if (selected_focus_ == -1 && !selected_center_ && (Mouse::isButtonPressed(Mouse::Left) || Mouse::isButtonPressed(Mouse::Right)))
	{
